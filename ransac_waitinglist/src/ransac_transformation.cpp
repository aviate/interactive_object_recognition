...
void RANSACTransformation::reconfigCallback(
    ransac_waitinglist::RANSACConfig &config,
    uint32_t level)
{
...
void RANSACTransformation::calcPointsDistance(
    const pcl::PointXYZ &point_source,
    const pcl::PointXYZ &point_destination,
    double &distance)
{
    distance = sqrt(
          (point_source.x - point_destination.x) * (point_source.x - point_destination.x)
        + (point_source.y - point_destination.y) * (point_source.y - point_destination.y)
        + (point_source.z - point_destination.z) * (point_source.z - point_destination.z)
    );
}

int RANSACTransformation::ransacUmeyama(
...
void RANSACTransformation::getInliers(
    const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &cloud_source_transformed,
    const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &cloud_destination,
...
void RANSACTransformation::umeyamaTransform (
...
bool RANSACTransformation::extractNeighbor(
    const pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud_source,
    ...
    pcl::PointXYZ &found_point)
{
    ...
    if (kdtree->nearestKSearch(
        searchPoint, 1, pointIdxNKNSearch,
