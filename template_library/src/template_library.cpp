...
void TemplateLibrary::loadClouds(const std::string &source_directory)
{
		...
		itr++)
	{
		...
		for (std::set<std::string>::iterator itr = directory_names.begin();
			itr != directory_names.end ();
			itr++)
		{
			directory_names_pair.insert(
				std::pair<std::string, std::string>(path.stem().string(), *itr)
			);
		}

		directory_names_full_list.insert(
			directory_names_pair.begin(),
			directory_names_pair.end()
		);
	}

	std::copy(
		directory_names_full_list.begin(), 
		directory_names_full_list.end(), 
...
std::vector<Template> TemplateLibrary::loadTemplates(const std::string &type)
{
		...
		itr++)
	{
			...
			itr++, i++)
		{
			...
			ROS_INFO_STREAM(directory << i << "cloud_rgb.pcd");
			ROS_INFO_STREAM(directory << i << "cloud_inliers.pcd");
			ROS_INFO_STREAM(directory << i << "image_no_plane.jpg");
...
cv::Mat TemplateLibrary::restoreCVMatNoPlaneFromPointCloud(
...
void TemplateLibrary::generatePlaneOutliers(
	const pcl::PointIndices &inliers,
	const uint &cloud_size,
...
void TemplateLibrary::planeSegmentation(
...
void TemplateLibrary::generateTemplateDataEuclidean(
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud_input,
	pcl::PointCloud<pcl::PointXYZLRegionF>::Ptr &cloud_input_with_inliers,
	pcl::PointIndices &template_inliers)
{
	pcl::ModelCoefficients coefficients;
...
void TemplateLibrary::generateTemplateDataActive(
