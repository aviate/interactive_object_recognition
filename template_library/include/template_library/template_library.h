...
#include <dense_reconstruction/DenseReconstruction.hpp>
...
struct Template
{
	...
	inline void setCloudPtr(
		const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloud_ptr)
	{
		cloud_ptr_.reset(new pcl::PointCloud<pcl::PointXYZRGB>(*cloud_ptr));
	}

	inline void setCloudWithInliersPtr(
		const pcl::PointCloud<pcl::PointXYZLRegionF>::Ptr &cloud_with_inliers_ptr)
	{
		cloud_with_inliers_ptr_.reset(
			new pcl::PointCloud<pcl::PointXYZLRegionF>(*cloud_with_inliers_ptr)
		);
...
public:
	...
	std::vector<Template> loadTemplates(
		const std::string &type = "template"
	);
...
private:
	void reconfigCallback(
		template_library::LibraryConfig &config,
		uint32_t level
	);
	...
	int getIndex(
		const pcl::PointCloud<pcl::PointXYZLRegionF>::Ptr &cloud
	);
