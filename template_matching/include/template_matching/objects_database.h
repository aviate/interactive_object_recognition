...
/**
 * @brief      Database for handling \ref ObjectData.
 */
...
public:
	...
	std::vector<ObjectData> databaseObjects();

private:
	void fit1DGaussianToData(
		const std::vector<float> &v, double &mean, double &stdev
	);
	...
	/**
	 * vector of object-pose pair vectors
	 * where each ObjectData is one training sample
	 */
