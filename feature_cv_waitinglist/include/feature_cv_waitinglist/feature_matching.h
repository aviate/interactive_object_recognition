...
{
	enum extractor_matcher {
		FLANN,
		BRUTEFORCE_HAMMING_1,
		BRUTEFORCE_HAMMING_2,
		BRUTEFORCE_L1,
		BRUTEFORCE_L2
	};

	enum feature_detector {
		SIFT_detector,
		SIFTGPU_detector,
		SURF_detector,
		FAST_detector,
...
private:
	...
	void filterMatchesUsingTracking(
		const std::vector<cv::KeyPoint> &template_keypoints,
		const std::vector<cv::KeyPoint> &search_keypoints,
		const std::vector<cv::DMatch> &matches,
		std::vector<cv::DMatch> &good_matches,
	...
	void filterMatchesUsingTrackingNN(
		const std::vector<cv::KeyPoint> &template_keypoints,
		const std::vector<cv::KeyPoint> &search_keypoints,
		const std::vector<cv::DMatch> &matches,
		std::vector<cv::DMatch> &good_matches,
	...
	void filterMatchesUsingTrackingBestMatch(
		const std::vector<cv::KeyPoint> &template_keypoints,
		const std::vector<cv::KeyPoint> &search_keypoints,
		const cv::Mat &template_descriptors,
	...
	ros::Time start_processing_time_;
	ros::Time end_processing_time_;
	ros::Time publish_time_;

	// class params
	double max_radius_search_dist_;
	double matching_distance_ratio_threshold_;
	double tracking_threshold_;
	double distance_factor_;
	double search_dist_;
	bool distinct_matches_;
	bool horizontal_matches_;
	bool tracking_matches_,drawing_template_;
	bool timing_debug_;
	bool outlier_removal_;
	bool retain_all_prev_matches_;
	bool double_check_tracking_matches_;
	bool reduce_search_area_;
	int horizontal_threshold_;
	int min_tracking_matches_;
	int feature_detector_;
	int feature_extractor_;
	int descriptor_matcher_;
	std::string feature_detector_parameter_value_;
	std::string feature_extractor_parameter_value_;
	...
	std::vector<cv::KeyPoint> prev_template_keypoints_;
	std::vector<cv::KeyPoint> prev_search_keypoints_;
	cv::Mat prev_template_descriptors_;
	cv::Mat prev_search_descriptors_;
