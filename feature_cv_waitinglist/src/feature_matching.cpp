...
typedef boost::shared_ptr<cv::FeatureDetector> FDPtr;
typedef boost::shared_ptr<cv::DescriptorExtractor> DEPtr;
typedef boost::shared_ptr<cv::DescriptorMatcher> DMPtr;
...
FeatureMatching::FeatureMatching() :
	...
	constructed_(false)
{
	feature_detector_ptrs_.push_back(FDPtr(new cv::SiftFeatureDetector()));
	feature_detector_ptrs_.push_back(FDPtr(new cv::SiftFeatureDetector()));
	feature_detector_ptrs_.push_back(FDPtr(new cv::SurfFeatureDetector()));
	feature_detector_ptrs_.push_back(FDPtr(new cv::FastFeatureDetector(FAST_threshold)));
	feature_detector_ptrs_.push_back(FDPtr(new cv::GridAdaptedFeatureDetector(
		new cv::FastFeatureDetector(), grid_max_number_keypoints))
	);
	feature_detector_ptrs_.push_back(FDPtr(new cv::MserFeatureDetector()));
	feature_detector_ptrs_.push_back(FDPtr(new cv::OrbFeatureDetector()));
	feature_detector_ptrs_.push_back(FDPtr(new cv::StarFeatureDetector()));

	feature_extractor_ptrs_.push_back(DEPtr(new cv::SiftDescriptorExtractor()));
	feature_extractor_ptrs_.push_back(DEPtr(new cv::SiftDescriptorExtractor()));
	feature_extractor_ptrs_.push_back(DEPtr(new cv::SurfDescriptorExtractor()));
	feature_extractor_ptrs_.push_back(DEPtr(new cv::OrbDescriptorExtractor()));
	feature_extractor_ptrs_.push_back(DEPtr(new cv::FREAK(FREAK_normalize_orientation)));
	feature_extractor_ptrs_.push_back(DEPtr(new cv::BriefDescriptorExtractor()));

	feature_matcher_ptrs_.push_back(DMPtr(new cv::FlannBasedMatcher()));
	feature_matcher_ptrs_.push_back(DMPtr(new cv::BFMatcher(cv::NORM_HAMMING, false)));
	feature_matcher_ptrs_.push_back(DMPtr(new cv::BFMatcher(cv::NORM_HAMMING2, false)));
	feature_matcher_ptrs_.push_back(DMPtr(new cv::BFMatcher(cv::NORM_L1, false)));
	feature_matcher_ptrs_.push_back(DMPtr(new cv::BFMatcher(cv::NORM_L2, false)));
...
void FeatureMatching::printParameterOptions(cv::Algorithm& cv_algorithm)
{
	...
	for (std::vector<std::string>::iterator itr_s = param_names.begin();
		itr_s!=param_names.end();
		itr_s++)
	{
...
void FeatureMatching::setParameter(
	cv::Algorithm& cv_algorithm,
	std::string param_name,
	std::string param_value)
{
	int param_type = cv_algorithm.paramType(param_name);
	ROS_INFO_STREAM(
		"setting [" << cv_algorithm.name()
			<< "] Parameter [" << param_name
			<< "] of type [" << param_type
			<< "] to value [" << param_value << "]"
	);
...
//  }
	...
	if ((config.feature_detector_parameter_value != feature_detector_parameter_value_)
		&& constructed_)
	{
		setParameter(
			*feature_detector_ptrs_[config.feature_detector],
			config.feature_detector_parameter_name,
			config.feature_detector_parameter_value
		);
	}

	if ((config.feature_extractor_parameter_value != feature_extractor_parameter_value_)
		&& constructed_)
	{
		setParameter(
			*feature_extractor_ptrs_[config.feature_extractor],
			config.feature_extractor_parameter_name,
			config.feature_extractor_parameter_value
		);
		prev_matches_.clear();    // the tracking uses previous matches.
		//TODO if there is a mismatch in matches type then the program will crash!
	}

	// binary descriptors need to use bruteforce or else it will crash
	if ((      config.feature_extractor == ORB_extractor
			|| config.feature_extractor == BRIEF_extractor
			|| config.feature_extractor == FREAK_extractor
		) && (
			   config.descriptor_matcher != BRUTEFORCE_HAMMING_1
			&& config.descriptor_matcher != BRUTEFORCE_HAMMING_2
		))
	{
		config.descriptor_matcher = BRUTEFORCE_HAMMING_1;
	}

	if ((	   config.feature_extractor == SURF_extractor
			|| config.feature_extractor == SIFTGPU_extractor
			|| config.feature_extractor == SIFT_extractor
		) && (
			   config.descriptor_matcher == BRUTEFORCE_HAMMING_1
			|| config.descriptor_matcher == BRUTEFORCE_HAMMING_2
		))
...
bool FeatureMatching::getDescriptorMatches(
...
bool FeatureMatching::getMatches(
	const cv::Mat &template_image,
	const cv::Mat &search_image,
	cv::Mat &matches_overlay,
	std::vector<cv::Point2f> &template_match_points,
	std::vector<cv::Point2f> &search_match_points)
{
	...
	return getMatches(
		template_image, search_image, matches_overlay,
		template_match_points, search_match_points,
		prev_template_match_points, prev_search_match_points
	);
}
...
//  this->findMatches (template_descriptors, search_descriptors, good_matches);
	...
	if (tracking_matches_)
	{
		for (uint i = 0; (i < prev_matches_.size()) && (i < matches.size()) && !reset_tracking; i++)
		{
			cv::line(
				matches_overlay,
				search_keypoints[matches[i].trainIdx].pt,
				prev_search_keypoints_[search_new_to_old_correspondences[i]].pt,
	...
	{
		cv::drawMatches(
			template_image, template_keypoints, search_image, search_keypoints,
			matches, matches_overlay, cv::Scalar(), cv::Scalar(), std::vector<char>(),
			cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS
		);
	}
	else
	{
		for (uint i = 0; i < matches.size(); i++)
		{
			...
			cv::line(
				matches_overlay,
				template_keypoints[matches[i].queryIdx].pt,
				search_keypoints[matches[i].trainIdx].pt,
				cv::Scalar(100, 255, 255)
			);
		}
...
//	drawInfoOnImg(matches.size(), matches_overlay);
	...
	for (std::vector<cv::DMatch>::const_iterator itr = matches.begin(); itr != matches.end(); itr++)
	{
		...
		{
			prev_template_match_points.push_back(
				prev_template_keypoints_[template_new_to_old_correspondences[itr-matches.begin()]].pt
			);
			prev_search_match_points.push_back(
				prev_search_keypoints_[search_new_to_old_correspondences[itr-matches.begin()]].pt
			);
		}
	}
...
bool FeatureMatching::getFeatures(
	const cv::Mat& image,
...
{
	ros::Duration processing_duration = end_processing_time_ - start_processing_time_;
	double processing_time = processing_duration.toSec() * 1000.0;
	...
	image_text_hz      << "Hz: "       << (1.0 / time_since_last_callback.toSec());
	image_text_time    << "Duration: " << processing_time << "ms";
	cv::putText(
		image,
		image_text_matches.str(),
		cvPoint(font_pos_x, font_pos_y + font_row_size * 0),
		cv::FONT_HERSHEY_PLAIN, 0.8, font_color, 1, CV_AA
	);
	cv::putText(
		image,
		image_text_hz.str(),
		cvPoint(font_pos_x, font_pos_y + font_row_size * 1),
		cv::FONT_HERSHEY_PLAIN, 0.8, font_color, 1, CV_AA
	);
	cv::putText(
		image,
		image_text_time.str(),
		cvPoint(font_pos_x, font_pos_y + font_row_size * 2),
		cv::FONT_HERSHEY_PLAIN, 0.8, font_color, 1, CV_AA
	);
}

void FeatureMatching::filterMatches(
...
void FeatureMatching::getHorizontalMatches(
	const std::vector<cv::KeyPoint> &template_keypoints,
...
void FeatureMatching::filterMatchesUsingTracking(
	const std::vector<cv::KeyPoint> &template_keypoints,
	const std::vector<cv::KeyPoint> &search_keypoints,
	const std::vector<cv::DMatch> &matches,
	std::vector<cv::DMatch> &good_matches,
...
void FeatureMatching::filterMatchesUsingTrackingNN(
	const std::vector<cv::KeyPoint> &template_keypoints,
	const std::vector<cv::KeyPoint> &search_keypoints,
	const std::vector<cv::DMatch> &matches,
	std::vector<cv::DMatch> &good_matches,
	...
	std::vector<uint> &search_new_to_prev_correspondence)
{
	...
	for (std::vector<cv::DMatch>::const_iterator itr_t1 = matches.begin(); itr_t1 != matches.end(); itr_t1++)
	{
		...
		for (std::vector<cv::DMatch>::const_iterator itr_t0 = prev_matches_.begin(); itr_t0 != prev_matches_.end(); itr_t0++)
		{
			...
			if (template_point_distance > tracking_threshold_)
				continue;

			const double search_point_distance = euclideanDist(search_pt, prev_search_pt);
			if (search_point_distance > tracking_threshold_)
				continue;
	...
	if (outlier_removal_)
	{
		removeOutliers(
			good_matches,
			min_point_distance_vec,
			template_new_to_prev_correspondence,
			search_new_to_prev_correspondence
		);
	}
}

void FeatureMatching::filterMatchesUsingTrackingBestMatch(
	const std::vector<cv::KeyPoint> &template_keypoints,
	const std::vector<cv::KeyPoint> &search_keypoints,
	const cv::Mat &template_descriptors,
	...
	std::vector<uint> &search_new_to_prev_correspondence)
{
	...
	for (std::vector<cv::DMatch>::const_iterator itr_t1 = matches.begin(); itr_t1 != matches.end(); itr_t1++)
	{
		...
		if (prev_local_template_descriptors.rows > 0)
		{
			...
			feature_matcher_ptrs_[descriptor_matcher_]->match(
				template_descriptors.row(itr_t1->queryIdx),
				prev_local_template_descriptors,
				point_template_matches
			);
			feature_matcher_ptrs_[descriptor_matcher_]->match(
				search_descriptors.row(itr_t1->trainIdx),
				prev_local_search_descriptors,
				point_search_matches
			);
...
void FeatureMatching::removeOutliers(
	std::vector<cv::DMatch> &matches,
	std::vector<double> &distances,
	std::vector<uint> &template_new_to_prev_correspondence,
	std::vector<uint> &search_new_to_prev_correspondence)
...
void FeatureMatching::SIFTGPUGetFeatures(const cv::Mat &input_image, std::vector<cv::KeyPoint> &keypoints, cv::Mat &descriptors)
{
#ifdef USE_SIFT_GPU
...
void FeatureMatching::findMatches(
