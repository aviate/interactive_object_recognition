...
{
	std::vector<FeatureMatching> normalized_points;
	...
	std::vector<FeatureMatching>::const_iterator itr;
...
void HomographyEstimation::normalizePoints(
	...
	Eigen::Vector3f  &transform)
{
	...
	for (uint i = 0; i < input_matches.size(); i++)
	{
		...
		featurematching.first = new FeatureDescriptorBase(input_matches[i].first);
		featurematching.second = new FeatureDescriptorBase(input_matches[i].second);
		output_matches.push_back(featurematching);
	}
...
//	ROS_ERROR_STREAM("avg sumes are: "<< avgSumX<< " and " << avgSumY);
	...
	for (uint i = 0; i < output_matches.size(); i++)
	{
		...
		sumDistances += sqrt(
			  feature->keyPointLocationX * feature->keyPointLocationX
			+ feature->keyPointLocationY * feature->keyPointLocationY
		);
//		ROS_ERROR_STREAM("X: "<< feature->keyPointLocationX<< " Y " << feature->keyPointLocationY);
	}
